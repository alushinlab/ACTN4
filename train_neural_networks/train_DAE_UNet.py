...
def mrc_generator(noisy_file_list, noiseless_file_list, box_length, start_idx, end_idx, batch_size=32):
	while True:
		...
		for i in range(start_idx, end_idx, batch_size):
			...
			batch_y = np.zeros((len(batch_files_noiseless), box_length, box_length, 1), dtype=np.float16)
			...
			for j in range(0, len(batch_files_noisy)):
				...
				if(noisy_data.shape == (box_length,box_length) and noiseless_data.shape == (box_length,box_length)):
...
def train_model(train_data, train_target, val_train, val_target):
	...
	checkpoint = keras.callbacks.ModelCheckpoint(OUTPUT_DIR_PATH +'k255e_CCC_{loss:.4f}_epoch_{epoch:02d}.h5', monitor='loss', verbose=0, save_best_only=True, mode='auto',save_weights_only=False, save_freq='epoch')
...
def train_model_generator(noise_list, noNoise_list):
	...
	autoencoder, encoder = create_model_dense(np.empty([1,BOX_DIM,BOX_DIM,1]),True, LEARNING_RATE)
...
model_save_name = OUTPUT_DIR_PATH + 'k255e_box256_CCC%s.h5'%str(''.join(str(np.abs(best_validation_loss))[:6].replace('.','p')))
print('Model finished training.\nSaving model as ' + model_save_name)
autoencoder_three.save(model_save_name)

import pickle
with open(OUTPUT_DIR_PATH + 'k255e_DAE_trainHistoryDict_fig.pkl', 'wb') as file_pi:
