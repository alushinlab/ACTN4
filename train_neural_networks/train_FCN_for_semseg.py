...
parser.add_argument('--input_noise_dir', type=str, help='directory with noisy projections')
parser.add_argument('--input_semseg_dir', type=str, help='directory with semseg projections')
parser.add_argument('--trained_DAE_path', type=str, help='final DAE network')
parser.add_argument('--proj_dim', type=str, help='projection dimensions')
...
parser.add_argument('--patience', type=int, help='number of epochs allowed to increase before stopping')
parser.add_argument('--epochs', type=int, help='total number epochs')
parser.add_argument('--batch_size', type=int, help='number of projects per batch')
parser.add_argument('--gpu_idx', type=int, help='which gpu to use')
...
NOISY_DIR_PATH = args.input_noise_dir              
SEMSEG_DIR_PATH = args.input_semseg_dir      
TRAINED_DAE_PATH = args.trained_DAE_path
BOX_DIM = int(args.proj_dim)                       
NUM_NOISE_PAIRS = int(args.numProjs)               
LEARNING_RATE = float(args.lr)                     
PATIENCE = int(args.patience)                      
EPOCHS = int(args.epochs)                          
BATCH_SIZE = int(args.batch_size)                  
TV_SPLIT = (100.0-float(args.tv_split))/100.0      
...
def mrc_generator(noisy_file_list, noiseless_file_list, box_length, start_idx, end_idx, batch_size=32):
	while True:
		...
		for i in range(start_idx, end_idx, batch_size):
			...
			batch_x = np.zeros((len(batch_files_noisy), box_length, box_length, 1), dtype=np.float16)
			batch_y = np.zeros((len(batch_files_noiseless), box_length, box_length, 3), dtype=np.float16)
			...
			for j in range(0, len(batch_files_noisy)):
				...
				if(noisy_data.shape == (box_length,box_length) and noiseless_data.shape == (3,box_length,box_length)):
					# Add the volume to the input array
					batch_x[j, :, :, 0] = noisy_data
					batch_y[j, :, :, 0] = noiseless_data[0]
					batch_y[j, :, :, 1] = noiseless_data[1]
					batch_y[j, :, :, 2] = noiseless_data[2]
...
def create_model_dense(training_data, full_training, lr):
	...
	x = layers.MaxPooling2D(pool_size=(4,4), padding='same')(x192)#[96x96x16]
	...
	x = layers.UpSampling2D((4,4))(x)#[192x192x64]
	...
	#semSeg.compile(optimizer=adam, loss='binary_crossentropy', metrics=['binary_crossentropy'])
	semSeg.summary()
	return semSeg
...
model_path = TRAINED_DAE_PATH
trained_DAE = keras.models.load_model(model_path, custom_objects={'CCC':CCC})

# Location of data
noise_folder = NOISY_DIR_PATH
noNoise_folder = SEMSEG_DIR_PATH

# Preload data into RAM or load training/validation data on the fly?
# Define the model
semSeg = create_model_dense(np.empty([1,BOX_DIM,BOX_DIM,1]),True, LEARNING_RATE)
...
if(PRELOAD_RAM):
	...
	noNoise_file_names = sorted(glob.glob(noNoise_folder+'*.mrcs'))
	...
	checkpoint = keras.callbacks.ModelCheckpoint(OUTPUT_DIR_PATH +'k255e_BCE_{loss:.4f}_epoch_{epoch:02d}.h5', monitor='val_loss', verbose=0, save_best_only=True, mode='min',save_weights_only=False, save_freq='epoch')
	train_gen = mrc_generator(noise_file_names, noNoise_file_names, box_length=BOX_DIM, start_idx=0, end_idx=train_num, batch_size=BATCH_SIZE)
	val_gen = mrc_generator(noise_file_names, noNoise_file_names, box_length=BOX_DIM, start_idx=train_num+1, end_idx=NUM_NOISE_PAIRS, batch_size=BATCH_SIZE)
	history = semSeg.fit(train_gen, epochs=EPOCHS, verbose=1, steps_per_epoch = train_num // BATCH_SIZE, validation_data = val_gen, validation_steps= (NUM_NOISE_PAIRS-train_num+1) // BATCH_SIZE, callbacks=[es,checkpoint])
	#return [semSeg, history, encoder]

################################################################################

model_save_name = OUTPUT_DIR_PATH + 'k255e_semSeg_binaryCrossEnt.h5'
...
import pickle
with open(OUTPUT_DIR_PATH + 'k255e_semSeg_trainHistoryDict.pkl', 'wb') as file_pi:
