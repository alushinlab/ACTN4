...
file_names_bare = sorted(glob.glob(folder+'*bare*.mrc'))
file_names_nine = sorted(glob.glob(folder+'*nine*.mrc'))
for i in range(0,len(file_names_bare)):
	actin_orig.append([EMData(file_names_bare[i]),EMData(file_names_nine[i])])

if(use_empirical_model):
	with mrcfile.open(path_to_empirical_model, 'r') as mrc:
		mrc = mrc.data
		empirical_pwrSpectrum = mrc[0]
		empirical_pwrSpectrum_std = mrc[1]
...
def launch_parallel_process(thread_idx):
	...
	for i in tqdm(range(0,num_per_proc),file=sys.stdout):
		...
		num_filaments = local_random_state.choice(max_fil_num+1,p=[0.15,0.6,0.2,0.05])
		a = 1.0/30
		r0 = local_random_state.choice(len(actin_orig), num_filaments,p=[0.5,a,a,a,a,a,a,a,a,a,a,a,a,a,a,a])
		if(num_filaments == 0):
			...
			if(use_empirical_model):
				...
				white_box = np.random.normal(0,0.4,[cropBox*2,cropBox*2])
				pnk_pw = np.multiply(empirical_pwrSpectrum+r14*empirical_pwrSpectrum_std, white_box)
				...
				target_noise = target_noise[cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
			...
			dae_target = np.zeros((cropBox*2,cropBox*2)); semSeg_target = np.zeros((3,cropBox*2,cropBox*2))
			bundle_idxs = []
			for j in range(1, num_filaments+1):
				r0_name = file_names_bare[r0[j-1]]
				rotated_actin = actin_orig[r0[j-1]][0].copy()
				rotated_actin_arm = actin_orig[r0[j-1]][1].copy()
				# handle bundles
				if(is_bundle):
				...
				proj_np_lp50 = EMNumPy.em2numpy(proj_eman_lp50)>0.1
				proj_np = EMNumPy.em2numpy(proj_eman)

				rotated_actin_arm.transform(t) # apply rotation and translation
				proj_eman_arm = rotated_actin_arm.project('standard',Transform()) # project
				proj_eman_lp50_arm = proj_eman_arm.process('filter.lowpass.gauss', {'apix':apix, 'cutoff_freq':1.0/lowpass_res})
				proj_eman_lp50_arm = EMNumPy.em2numpy(proj_eman_lp50_arm)>0.1
				proj_np_arm = EMNumPy.em2numpy(proj_eman_arm)

				dae_target = dae_target+ proj_np+proj_np_arm
				
				semSeg_target[1] = semSeg_target[1] + proj_np
				semSeg_target[2] = semSeg_target[2] + proj_np_arm
			
			# Generate noisy image
			target_proj = (dae_target - np.mean(dae_target)) / np.std(dae_target)
			r7 = local_random_state.uniform(defoc_low, defoc_high) #defocus
			target_eman = EMNumPy.numpy2em(target_proj)
			if(use_empirical_model):
				r14 = local_random_state.normal(0,0.5)
				target_eman.process_inplace('math.simulatectf',{'ampcont':ampCont,'apix':apix,'bfactor':bfact,'cs':cs,'defocus':r7,'voltage':voltage})
				target_noise = EMNumPy.em2numpy(target_eman)
				white_box = np.random.normal(0,0.4,[cropBox*2,cropBox*2])
				pnk_pw = np.multiply(empirical_pwrSpectrum+r14*empirical_pwrSpectrum_std, white_box)
				pnk_pw_shift = np.fft.ifftshift(pnk_pw)
				pink_noise_box = np.fft.ifft2(pnk_pw_shift).real
				pink_noise_box = ((pink_noise_box - np.average(pink_noise_box)) / np.std(pink_noise_box))
				target_noise = np.fft.ifft2(np.fft.fft2((np.random.gamma(1.5,0.35)+0.6)*target_noise) +np.fft.fft2(pink_noise_box)).real
				target_noise = target_noise[cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
				target_noise = (target_noise - np.mean(target_noise)) / np.std(target_noise) # normalize	
			...
			target_proj = target_proj[cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
			with mrcfile.new(noNoise_outputDir + 'actin_rotated%05d.mrc'%(i+num_per_proc*thread_idx), overwrite=True) as mrc:
				mrc.set_data(target_proj.astype('float32'))
			
			semSeg_target[1][semSeg_target[1]>0.0001] =1
			semSeg_target[2][semSeg_target[2]>0.0001] =1
			semSeg_target[1][(semSeg_target[1] + semSeg_target[2])>=2] = 0
			semSeg_target[0] = (-1*(np.clip((semSeg_target[1] + semSeg_target[2]),0,1) - 1))
			semMap_cropped = np.zeros((3,cropBox,cropBox))
			semMap_cropped[0] = semSeg_target[0][cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
			semMap_cropped[1] = semSeg_target[1][cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
			semMap_cropped[2] = semSeg_target[2][cropBox-half_box:cropBox+half_box, cropBox-half_box:cropBox+half_box]
			with mrcfile.new(semMap_outputDir + 'actin_rotated%05d.mrcs'%(i+num_per_proc*thread_idx), overwrite=True) as mrc:
				mrc.set_data(semMap_cropped.astype('float32'))
